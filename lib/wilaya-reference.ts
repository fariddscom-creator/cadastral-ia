...
export function wilayaFromFileName(fileName: string): { code: string; name: string } | null {
  const match = fileName.match(/^(\d{1,2})\./);
  if (!match) return null;
  const code = match[1].padStart(2, '0');
  const name = wilayaNameFromCode(code);
  return name ? { code, name } : null;
}

export function wilayaNameFromCode(value: string | number): string | null {
  return wilayas[String(value).padStart(2, '0')] || null;
}
